...
def pychell_rvs_main(user_input_options, user_model_blueprints):
    ...
    for order_num in range(gpars['n_orders']):
        ...
        for iter_num in range(gpars['n_template_fits']):
            ...
            np.savez(gpars['run_output_path'] + 'Order' + str(order_num+1) + os.sep + 'RVs' + os.sep + gpars['full_tag'] + '_rvs_ord' + str(order_num+1) + '.npz', rvs, rvs=rvs, rvs_cd=rvs_cd, rvs_xcorr=rvs_xcorr, BJDS_cd=gpars['BJDS_nightly'], BJDS=gpars['BJDS'], n_obs_nights=gpars['n_obs_nights'], bisector_spans=bispans_all)
...
def fit_spectra_mcmc(forward_models, iter_num, templates_dict, gpars):

    # Loop over spectra
    for spec_num in range(gpars['n_spec']):

        forward_models[spec_num].disable(par_names=['blaze_base_quad', 'blaze_base_lin', 'blaze_base_zero', 'lsf_width', 'fringing_1_d', 'fringing_1_fin', 'water_depth', 'methane_depth', 'nitrous_oxide_depth'])
        for i in range(gpars['n_blaze_splines']):
            forward_models[spec_num].disable(par_names=['blaze_spline_' + str(i+1)])

        # Extract numpy arrays form parameter objects
        par_names, par_init_vals, par_min_vals, par_max_vals, par_varies_bool, par_mcmc_scales = forward_models[spec_num].initial_parameters.to_numpy(kind='all')
        par_varies_inds = np.where(par_varies_bool)[0]
        par_non_varies_inds = np.where(~par_varies_bool)[0]
        n_vary_pars = len(par_varies_inds)
        par_names_vary = par_names[par_varies_inds]
        par_non_vary_vals = par_init_vals[par_non_varies_inds]
        par_init_vals_vary = par_init_vals[par_varies_inds]
        par_min_vals_vary = par_min_vals[par_varies_inds]
        par_max_vals_vary = par_max_vals[par_varies_inds]
        par_mcmc_scales_vary = par_mcmc_scales[par_varies_inds]

        # Generate a block of Gaussian noise and scale it / center it appropriately
        n_walkers = gpars['mcmc_walkers_factor'] * n_vary_pars
        noise_block = np.random.randn(n_vary_pars * n_walkers).reshape((n_walkers, n_vary_pars))
        mcmc_init_pars = noise_block * (np.tile(par_mcmc_scales_vary, (n_walkers, 1))) + np.tile(par_init_vals_vary, (n_walkers, 1))

        # Initialize the MCMC sampler (affine invariant; goodman and weare 2010)
        sampler = emcee.EnsembleSampler(n_walkers, n_vary_pars, log_prob_model, args=[forward_models[spec_num], templates_dict, par_min_vals_vary, par_max_vals_vary, par_varies_inds, par_non_varies_inds, par_non_vary_vals, gpars], threads=gpars['n_threads'])

        # Launch an initial burn-in phase to start from a true random point
        print('Initiating burn-in phase (' + str(gpars['mcmc_burn_in_steps'])+ ' steps) for spectrum ' + str(spec_num+1) + ' of ' + str(gpars['n_spec']))
        ti = time.time()
        #pos, _, _ = sampler.run_mcmc(mcmc_init_pars, gpars['mcmc_burn_in_steps'])
        # We'll track how the average autocorrelation time estimate changes
        index = 0
        autocorr = np.empty(gpars['mcmc_burn_in_steps'], dtype=float)

        # This will be useful to testing convergence
        old_tau = np.inf
        for sample in sampler.sample(mcmc_init_pars, iterations=gpars['mcmc_burn_in_steps'], progress=True):
            if sampler.iteration % 10:
                line =  'MCMC Burn-in ... Iteration ' + str(sampler.iteration) + ' of ' + str(gpars['mcmc_burn_in_steps'])
        ...
        print('Time to complete burn-in phase: ' + dt + ' min')

        # Reset the sampler properties, position is already saved above and is all we care about
        sampler.reset()

        pos = sample

        # Launch the main phase from the attained random point
        print('Initiating main phase (' + str(gpars['mcmc_main_steps'])+' steps) for spectrum ' + str(spec_num+1) + ' of ' + str(gpars['n_spec']))
        ti = time.time()
        #pos, prob, state = sampler.run_mcmc(pos, gpars['mcmc_main_steps'])
        # We'll track how the average autocorrelation time estimate changes
        index = 0
        autocorr = np.empty(gpars['mcmc_main_steps'], dtype=float)

        # This will be useful to testing convergence
        old_tau = np.inf
        for sample in sampler.sample(pos, iterations=gpars['mcmc_main_steps'], progress=True):
        ...
        tf = time.time()
        dt = str(round((tf - ti) / 60, 3))
        print('Time to complete main phase: ' + dt + ' min')

        # Do Jonathan magic to get the best pars, ignoring bad chains
        acc = sampler.acceptance_fraction
        acc_med = np.nanmedian(acc)
        acc_mad = pcmath.mad(acc)
        acceptance_rate_threshold = 0.3
        chain_nsigma_threshold = 3.0
        good_chains = np.where(((np.abs((acc - acc_med) / acc_mad) < chain_nsigma_threshold) | (acc > acceptance_rate_threshold)) & (np.nansum(sampler.lnprobability, axis=1) != -np.inf))
        good_chains = np.reshape(good_chains, np.size(good_chains))
        flat_chains = sampler.chain[good_chains, :, :].reshape(np.size(good_chains) * gpars['mcmc_main_steps'],n_vary_pars)
        best_pars_vary = sampler.chain[1, np.argmax(sampler.lnprobability[1,:]), :]
        best_pars = np.empty(len(par_names), dtype=float)
        best_pars[par_varies_inds] = best_pars_vary
        best_pars[par_non_varies_inds] = par_non_vary_vals

        # Store the best fit mcmc parameter results
        forward_models[spec_num].mcmc_best_fit_pars = pcmodelcomponents.Parameters.from_numpy(names=par_names, values=best_pars, minvs=par_min_vals, maxvs=par_max_vals, varies=par_varies_bool, mcmcscales=par_mcmc_scales)
        #forward_models[spec_num].mcmc_best_fit_pars.append(flat_chains)

        # Build the best fit forward model from the mcmc parameters
        forward_models[spec_num].mcmc_wavelength_solution, forward_models[spec_num].mcmc_model = forward_models[spec_num].build_full(forward_models[spec_num].mcmc_best_fit_pars, templates_dict, gpars)

        # Compute the residuals between the data and model.
        # If iter_num is zero, don't flag the stellar lines here as bad, otherwise they won't get added properly.
        forward_models[spec_num].mcmc_residuals = forward_models[spec_num].data.flux - forward_models[spec_num].mcmc_model

        # Output a plot
        forward_models[spec_num].plot('mcmc', templates_dict, gpars)
        fname = gpars['run_output_path'] + 'Order' + str(forward_models[spec_num].order_num+1) + os.sep + 'Fits' + os.sep + gpars['full_tag'] + 'mcmc_corner_plot_spec' + str(spec_num+1) + '_ord' + str(forward_models[spec_num].order_num+1) + '.png'
        corner.corner(flat_chains, truths=best_pars_vary, labels=par_names_vary, truth_color='red')
        plt.savefig(fname, dpi=300)
        plt.close()


# gp only contains the varied parameters
def log_prob_model(gp, fwm, templates_dict, par_min_vals_vary, par_max_vals_vary, vary_inds, non_vary_inds, non_vary_vals, gpars):

    # If out of bounds, return -inf
    if np.any(gp < par_min_vals_vary) or np.any(gp > par_max_vals_vary):
        return -np.inf

    # Join the varied params with the non varied params
    n_pars = vary_inds.size + non_vary_inds.size
    gp_new = np.empty(n_pars, dtype=np.float64)
    gp_new[vary_inds] = gp
    gp_new[non_vary_inds] = non_vary_vals

    gp_ = pcmodelcomponents.Parameters.from_numpy(gpars['par_names'], values=gp_new)

    # Generate the forward model
    wave_lr, model_lr = fwm.build_full(gp_, templates_dict, gpars)

    # Normal RMS
    wdiffs2 = (fwm.data.flux - model_lr)**2 / fwm.data.flux_unc * fwm.data.badpix
    good = np.where(np.isfinite(wdiffs2) & (fwm.data.badpix == 1))[0]
    ...
    wresiduals2[ss[-20:]] = 0.0
    n_use_pix = good.size - 20

    chi2 = np.nansum(wresiduals2) / (n_use_pix - 1)
    loglike = -chi2 / 2
...
# Options: star, lab
def optimize_templates_wobble(forward_models, templates_dict, iter_num, order_num, gpars):

    # Weight templates by 1 / fit quality from RMS
    rms = pcforwardmodels.ForwardModel.extract_rms(forward_models, iter_num, gpars)
    rms_weights = 1 / rms**2
    
    # Extract the bc vels and current best fit stellar vels
    bary_corrs = gpars['bary_corrs']

    # The current stellar template
    current_stellar_template = templates_dict['star']

    # Extract numpy arrays from the forward models to turn into torch tensors
    raw_models_partial = np.empty(shape=(gpars['n_model_pix'], gpars['n_spec']), dtype=np.float64)
    data = np.empty(shape=(gpars['n_data_pix'], gpars['n_spec']), dtype=np.float64)
    waves_lr = np.empty(shape=(gpars['n_data_pix'], gpars['n_spec']), dtype=np.float64)
    waves_hr_star_shifted = np.empty(shape=(gpars['n_model_pix'], gpars['n_spec']), dtype=np.float64) # barycenter correction dependent, the shifted hr stellar grids
    badpix = np.empty(shape=(gpars['n_data_pix'], gpars['n_spec']), dtype=np.float64)
    weights = np.empty(shape=(gpars['n_data_pix'], gpars['n_spec']), dtype=np.float64)

    # Loop over spectra and extract to the above arrays
    lam_lefts = np.empty(gpars['n_spec'], dtype=np.float64)
    lam_rights = np.empty(gpars['n_spec'], dtype=np.float64)
    for ispec in range(gpars['n_spec']):

        # Get the pars for this iteration
        pars = forward_models[ispec].best_fit_pars[iter_num]

        
        raw_models_partial[:, ispec], blaze[:, ispec], lsf[:, ispec] = forward_models[ispec].build_1(pars, templates_dict, gpars)

        # The data
        data[:, ispec] = np.copy(forward_models[ispec].data.flux)
        bad = np.where(~np.isfinite(data[:, ispec]))[0]
        if bad.size > 0:
            data[bad, ispec] = 0

        # The low resolution wavelength solution grids
        waves_lr[:, ispec] = forward_models[ispec].wavelength_solutions[:, iter_num]
        
        lam_lefts[ispec] = waves_lr[gpars['pix_left'], ispec] * np.exp(-pars[forward_models[ispec].models_dict['star'].par_names[0]].value / cs.c)
        lam_rights[ispec] = waves_lr[gpars['pix_right'], ispec] * np.exp(-pars[forward_models[ispec].models_dict['star'].par_names[0]].value / cs.c)
        

        # We need the shifted stellar wavelength grids used for each observation.
        waves_hr_star_shifted[:, ispec] = current_stellar_template[:, 0] * np.exp(pars[forward_models[ispec].models_dict['star'].par_names[0]].value / cs.c)

        # The weights
        badpix[:, ispec] = np.copy(forward_models[ispec].data.badpix)
        weights[:, ispec] = np.copy(forward_models[ispec].data.badpix)
        
        
    star_wave_left, star_wave_right = np.min(lam_lefts), np.max(lam_rights)
    ...
    # Constants
    waves_lr = torch.from_numpy(waves_lr)
    waves_hr_star_shifted = torch.from_numpy(waves_hr_star_shifted)
    raw_models_partial = torch.from_numpy(raw_models_partial)
    lsf = torch.from_numpy(lsf)
    weights = torch.from_numpy(weights)
    data = torch.from_numpy(data)
    blaze = torch.from_numpy(blaze)
    wave_hr_master = torch.from_numpy(current_stellar_template[:, 0])
    
    # Star is multiplicative because we are modeling the entire stellar flux.
    # Lab frame is additive.
    star_flux = torch.nn.Parameter(torch.from_numpy(current_stellar_template[:, 1].astype(np.float64)))
    if 'lab' in gpars['templates_to_optimize']:
        lab_flux = torch.nn.Parameter(torch.from_numpy(np.zeros(gpars['n_model_pix'], dtype=np.float64)))
    else:
        lab_flux = torch.from_numpy(np.zeros(gpars['n_model_pix'], dtype=np.float64)) # Additive here
        
    # Create the Torch model object
    model = pcforwardmodels.StarSolver(star_flux, lab_flux, raw_models_partial, waves_lr, waves_hr_star_shifted, weights, lsf, data, wave_hr_master, blaze)
    ...
    templates_optimized = OrderedDict()
    
    # Optimizing star
    if 'star' in gpars['templates_to_optimize']:
        new_star_flux = model.star_flux.detach().numpy()
        bad = np.where(new_star_flux > 1.0)[0]
        if bad.size > 0:
            new_star_flux[bad] = 1.0
        bad = np.where((current_stellar_template[:, 0] < star_wave_left) | (current_stellar_template[:, 0] > star_wave_right))[0]
        if bad.size > 0:
            new_star_flux[bad] = 1.0
        templates_optimized['star'] = np.array([current_stellar_template[:, 0], new_star_flux], dtype=np.float64).T
    # Optimizing Lab
    if 'lab' in gpars['templates_to_optimize']:
        new_lab_flux = model.lab_flux.detach().numpy()
        templates_optimized['lab'] = np.array([current_stellar_template[:, 0], new_lab_flux], dtype=np.float64).T

    return templates_optimized
